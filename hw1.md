...
MNT: Ситуация A[i - k] > A[i - 1] для какого-то k невозможна, так как внешний цикл каждый раз на i-е место ставит минимальный из элементов A[i], A[i + 1], ..., A[n - 1]. Поэтому на (i - k)-м месте не может оказаться элемент, который больше элемента, стоящего впереди него на (i - 1)-м месте.  
Иными словами, A[i - k] <= A[i - k + 1], ..., A[i - 1], ..., A[n - 1].  
